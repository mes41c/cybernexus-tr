...
const feedUrls = [
    ...
    'https://shiftdelete.net/guvenlik/feed'
];
...
// ==============================================================================
// === DÖNÜŞTÜRÜLMÜŞ VERİTABANI ENDPOINT'LERİ (4 ADET) ===
// ==============================================================================
...
// ==============================================================================
// === DÖNÜŞÜM SONA ERDİ (GERİ KALAN KODLAR OLDUĞU GİBİ KALDI) ===
// ==============================================================================
    ...
    try {
        switch (provider) {
            case 'gemini':
                ...
                const result = await geminiModel.generateContentStream(title); 
                for await (const chunk of result.stream) {
                    res.write(chunk.text());
                }
                res.end();
...
`;
    ...
    try {
        ...
        if (startIndex === -1 || endIndex === -1) {
            console.error("AI Yanıtı (Hatalı JSON):", generatedText);
        ...
        if (caseType === 'private') {
            const safeUserId = path.basename(anonymousUserId);
            saveDirectory = path.join(casesDirectory, 'private', safeUserId); 
        } else {
            saveDirectory = path.join(casesDirectory, 'common'); 
        }
...
app.post('/api/cases/:caseId/evaluate', async (req, res) => {
    ...
    try {
        ...
        const caseFileContent = await fs.promises.readFile(caseFilePath, 'utf8');
        const caseData = JSON.parse(caseFileContent);
        const groundTruth = caseData.news_article_text[language]; 
        ...
        for (const provider of providerPriority) {
            ...
            try {
                ...
                if (provider === 'gemini') {
                    ...
                    const baseURL = provider === 'deepseek' ? '[https://api.deepseek.com/v1](https://api.deepseek.com/v1)' : null;
                    const openai = new OpenAI({ apiKey, ...(baseURL && { baseURL }) });
                    const modelName = provider === 'deepseek' ? 'deepseek-chat' : 'gpt-4o-mini';
                    const response = await openai.chat.completions.create({
                        model: modelName,
